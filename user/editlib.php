...
function useredit_shared_definition(&$mform, $editoroptions, $filemanageroptions, $user) {
    ...
    // PLATINUM EDIT
    // SQL for finding necessary field data
    ...
    $sql .= 'WHERE uif.shortname = :fieldname ';
    ...
    if ($field = $DB->get_record_sql($sql, ['userid' => $user->id, 'fieldname' => 'address2'])) {
        ...
        $fieldobject = new $classname($field->id, $user->id, $field);
    ...
    // PLATINUM EDIT
    if ($field = $DB->get_record_sql($sql, ['userid' => $user->id, 'fieldname' => 'state'])) {
        require_once($CFG->dirroot . '/user/profile/field/' . $field->datatype . '/field.class.php');
        $classname = 'profile_field_' . $field->datatype;
        $field->hasuserdata = !empty($field->hasuserdata);
        /** @var profile_field_base $fieldobject */
    ...
    if ($field = $DB->get_record_sql($sql, ['userid' => $user->id, 'fieldname' => 'postalcode'])) {
        require_once($CFG->dirroot . '/user/profile/field/' . $field->datatype . '/field.class.php');
        $classname = 'profile_field_' . $field->datatype;
        $field->hasuserdata = !empty($field->hasuserdata);
        /** @var profile_field_base $fieldobject */
