...
function xmldb_certificate_upgrade($oldversion=0) {
        ...
        if ($dbman->table_exists('certificate_linked_modules')) {
            ...
            if ($certs = $DB->get_records('certificate')) {
                foreach ($certs as $cert) {
                    ...
                    if (!$availability = json_decode($cm->availability)) {
    ...
    if ($oldversion < 2019041100) {
        ...
        // Ensure all certificate modules have standard certificate type.
        // Users can still download old certificates, but not reissue new ones, thus custom certificate types are no
        // longer needed.
        $DB->execute('UPDATE {certificate} SET certificatetype = :certificatetype', ['certificatetype' => 'A4_non_embedded']);

        upgrade_mod_savepoint(true, 2019041100, 'certificate');
    }
