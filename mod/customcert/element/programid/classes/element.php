...
namespace customcertelement_programid;

defined('MOODLE_INTERNAL') || die();

/**
 * The customcert element programid core interaction API.
 *
 * @package    customcertelement_programid
 * @copyright  2019 MLC
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
...
class element extends \mod_customcert\element {
    ...
    protected function get_program_id() : string {
        ...
        $programs = $DB->get_records('customcert_program_ids', ['customcertid' => $cm->instance]);
        $programlist = implode(', ',
            array_map( function($x) { if (!empty($x->programid)) {
                return $x->programid . '[' . $x->programtime . ' ' . get_string('hours') . ']' ;
            } }, $programs));
