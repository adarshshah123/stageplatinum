...
class mod_customcert_mod_form extends moodleform_mod {
    ...
    public function definition() {
        ...
        // program ids.
        $mform->addElement('header', 'programs', get_string('programs', 'customcert'));

        $programrepeatarray = array();
        $programrepeatarray[] = $mform->createElement('text', 'programids',
            get_string('programids', 'mod_customcert'));
        $mform->setType('programids', PARAM_ALPHANUMEXT);
        $programrepeatarray[] = $mform->createElement('text', 'programtime',
            get_string('programtime', 'mod_customcert'));
        $mform->setType('programtime', PARAM_INT);
        $programrepeatsname = 'program_repeats';

        if (!empty($this->_instance)) {
            $this->_programs = $DB->get_records('customcert_program_ids', ['customcertid' => $this->_instance], 'id DESC');
            $numprograms = count($this->_programs);
        } else {
            $this->_programs = [];
            $numprograms = 1;
        }

        $this->repeat_elements($programrepeatarray, $numprograms, array(), $programrepeatsname,
            'programids_add_fields', 1,
            get_string('programidsaddfields', 'mod_customcert'), true);
        $mform->addHelpButton('programids[0]', 'programids',
            'mod_customcert', '', true);
        $mform->registerNoSubmitButton('programids_add_fields');

        // CTEC Program.
        $mform->addElement('header', 'headerctecprogram', get_string('ctecprograms', 'customcert'));
        $ctegprogramsrepeatarray = array();
        $ctegprogramsrepeatarray[] = $mform->createElement('text', 'ctecprogramid', get_string('ctecprogramid', 'mod_customcert'));
        $mform->setType('ctecprogramid', PARAM_ALPHANUMEXT);

        $ctecprogramrepeatsname = 'ctecprogram_repeats';

        if (!empty($this->_instance)) {
            $this->_ctecprograms = $DB->get_records('customcert_ctecprogram', ['customcertid' => $this->_instance], 'id DESC');
            $numctecprograms = count($this->_ctecprograms);
        } else {
            $this->_ctecprograms = [];
            $numctecprograms = 1;
        }

        $this->repeat_elements($ctegprogramsrepeatarray, $numctecprograms, array(), $ctecprogramrepeatsname,
            'ctecprogramid_add_fields', 1,
            get_string('ctecprogramidaddfields', 'mod_customcert'), true);
        $mform->addHelpButton('ctecprogramid[0]', 'ctecprogramid', 'mod_customcert', '', true);

        $mform->registerNoSubmitButton('ctecprogramid_add_fields');
    ...
    public function data_preprocessing(&$defaultvalues) {
        ...
        if (!empty($defaultvalues['update'])) {
            ...
            // set values for program ids.
            if (count($this->_programs)) {
                $key = 0;
                foreach ($this->_programs as $program) {
                    $defaultvalues['programids['.$key.']'] = $program->programid;
                    $defaultvalues['programtime['.$key.']'] = $program->programtime;
                    $key++;
                }
            }
            // set values for CTE program ids.
            if (count($this->_ctecprograms)) {
                $key = 0;
                foreach ($this->_ctecprograms as $program) {
                    $defaultvalues['ctecprogramid['.$key.']'] = $program->ctecprogramid;
                    $key++;
                }
            }
        } else {
