...
class element_helper {
    ...
    /**
     * Helper function that returns the related certificate for this element.
     *
     * @param int $elementid The element id
     * @return \stdClass Record of the certificate
     */
    public static function get_certificate(int $elementid) {
        global $DB;

        $sql = "SELECT cert.*
                  FROM {customcert} cert
            INNER JOIN {customcert_pages} cp
                    ON cert.templateid = cp.templateid
            INNER JOIN {customcert_elements} ce
                    ON cp.id = ce.pageid
                 WHERE ce.id = :elementid
              GROUP BY cert.id";
        $certificate = $DB->get_record_sql($sql, array('elementid' => $elementid));
