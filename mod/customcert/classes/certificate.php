...
class certificate {
     ...
     */
    public static function save_program_ids($id, $programids, $programtime) {
        global $DB;

        $DB->delete_records('customcert_program_ids', ['customcertid' => $id]);
        $index = 0;
        foreach ($programids as $programid) {
            $program = new \stdClass();
            $program->customcertid = $id;
            $program->programid = $programid;
            $program->programtime = $programtime[$index];
            $program->timecreated = time();
            $DB->insert_record('customcert_program_ids', $program);
            $index++;
        }
    }
     ...
     * @throws \dml_exception
     */
    public static function save_ctecprogram_ids($id, $programids) {
        global $DB;

        $DB->delete_records('customcert_ctecprogram', ['customcertid' => $id]);
        foreach ($programids as $programid) {
            $program = new \stdClass();
            $program->customcertid = $id;
            $program->ctecprogramid = $programid;
            $program->timecreated = time();
            $DB->insert_record('customcert_ctecprogram', $program);
        }
    }
}
